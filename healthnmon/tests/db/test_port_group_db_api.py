...
class PortGroupDbApiTestCase(test.TestCase):
    ...
    def test_port_group_save(self):
        ...
        self.assertTrue(portgroup.id == pgs[indexOfThePG].id, 'Portgroup id is invalid')
        self.assertTrue(portgroup.name == pgs[indexOfThePG].name, ' PortGroup name is invalid')
        self.assertTrue(portgroup.note == pgs[indexOfThePG].note, 'PortGroup Note is invalid')
        self.assertTrue(portgroup.cost.value == pgs[indexOfThePG].cost.value, 'PortGroup Value is invalid')
        self.assertTrue(portgroup.cost.units == pgs[indexOfThePG].cost.units, 'PortGroup Units is invalid')
        self.assertTrue(portgroup.resourceManagerId == pgs[indexOfThePG].resourceManagerId, 'PortGroup resourceManagerId is invalid')
        self.assertTrue(portgroup.get_type() == pgs[indexOfThePG].get_type(), 'PortGroup type is invalid')
        self.assertTrue(portgroup.virtualSwitchId == pgs[indexOfThePG].virtualSwitchId, 'PortGroup virtualSwitchId is invalid')
        self.assertTrue(portgroup.vmHostId == pgs[indexOfThePG].vmHostId, 'PortGroup vmHostId is invalid')

        self.assertTrue(len(pgs) == 1,
                        'port groups all returned valid number of list')

    def test_port_group_save_none(self):
        self.assertTrue(api.port_group_save(get_admin_context(), None) == None, 'Port group save none should save nothing')
    ...
    def test_port_group_get_by_id_empty_list(self):
        ...
        self.assertTrue(pgs == None, 'The returned Portgroups list should be empty')

    def test_port_group_delete_empty_list(self):
        self.assertTrue(api.port_group_delete_by_ids(get_admin_context(), None) == None, 'This method should return None')
    ...
    def test_port_group_delete(self):
        ...
        self.assertTrue(portgroups == None or len(portgroups) == 0,
    ...
    def test_timestamp_columns(self):
        ...
        portGrp_queried = api.port_group_get_by_ids(get_admin_context(), [portGrp.get_id()])[0]
        ...
        epoch_after = utils.get_current_epoch_ms()
        portGrp_queried = api.port_group_get_by_ids(get_admin_context(), [portGrp.get_id()])[0]
        self.assert_(portGrp_modified.get_createEpoch() == portGrp_queried.get_createEpoch())
        self.assert_(test_utils.is_timestamp_between(
