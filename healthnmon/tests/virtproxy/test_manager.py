...
from healthnmon.virtproxy import manager
from healthnmon.profiler import helper
from healthnmon.virtproxy import driver
...
class HealthnMonvirtProxyManagerTestCase(test.TestCase):

    """Test case for scheduler manager"""

    manager_cls = manager.HealthnMonVirtProxyManager
    driver_cls_name = 'healthnmon.virtproxy.driver.Healthnmon'
    driver_cls = driver.Healthnmon

    def setUp(self):
        super(HealthnMonvirtProxyManagerTestCase, self).setUp()
        self.flags(hypervisor_type="fake")
        self.flags(healthnmon_virtproxy_driver=self.driver_cls_name)
    ...
    def _createInvCache(self):
        self._createCache()
        self.mox.ReplayAll()
        self.manager = self.manager_cls('localhost')
    ...
    def test_2_incorrect_init(self):
        try:
            self.manager = \
                self.manager_cls('localhost',
                                 'nova.healthnmon.driver.Healthnmon1')
        except SystemExit, e:
            ...
            self.fail('unexpected exception: %s' % io.getvalue())
        else:
    ...
    def test_register_flag(self):
        ...
        cfg.ConfigOpts.__getattr__('healthnmon_virtproxy_driver').\
            AndRaise(cfg.NoSuchOptError('healthnmon_virtproxy_driver'))
        cfg.ConfigOpts.__getattr__('perfmon_refresh_interval').\
            AndRaise(cfg.NoSuchOptError('perfmon_refresh_interval'))
        cfg.ConfigOpts.__getattr__('healthnmon_virtproxy_topic').\
            AndRaise(cfg.NoSuchOptError('healthnmon_virtproxy_topic'))
