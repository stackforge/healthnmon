...
LOG = logging.getLogger('healthnmon.virtproxy.manager')

manager_opts = [
    cfg.StrOpt('healthnmon_virtproxy_driver',
               default='healthnmon.virtproxy.driver.Healthnmon',
...
def register_flags():
    try:
        CONF.healthnmon_virtproxy_driver
    ...
    try:
        CONF.healthnmon_virtproxy_topic
    except cfg.NoSuchOptError:
...
class HealthnMonVirtProxyManager(manager.Manager):
    ...
    def __init__(
        self,
        host=None,
        healthnmon_virtproxy_driver=None,
        *args,
        **kwargs
    ):
        self.host_name = host
        if not healthnmon_virtproxy_driver:
            healthnmon_virtproxy_driver = CONF.healthnmon_virtproxy_driver
        LOG.info("Initializing healthnmon. \
            Loading driver %s" % healthnmon_virtproxy_driver)
        try:
            self.driver = \
                utils.check_isinstance(
                    importutils.import_object(
                        healthnmon_virtproxy_driver, host=self.host_name),
                    driver.Healthnmon)
        ...
        super(HealthnMonVirtProxyManager, self).__init__(*args, **kwargs)
